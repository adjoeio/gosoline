...
const (
	...
	metricNameConsumerRetryGetCount  = "ConsumerRetryGetCount"
	metricNameConsumerRetryPutCount  = "ConsumerRetryPutCount"
	...
	return func(ctx context.Context) error {
		...
		for msg := range input.Data() {
			if retryId, ok := msg.Attributes[AttributeRetryId]; ok {
				...
				c.logger.WithContext(newCtx).WithFields(log.Fields{
