...
func NewProducerRetryDaemon(ctx context.Context, config cfg.Config, logger log.Logger, name string, metadata RetryMetadata) (ProducerRetryDaemon, error) {
	...
	output, err := ProvideConfigurableOutput(ctx, config, logger, name)
	if err != nil {
		return nil, fmt.Errorf("can not create retry handler: %w", err)
	}
...
func (p *producerRetryDaemon) retry(ctx context.Context, messages ...WritableMessage) error {
	for _, writableMessage := range messages {
		...
		p.logger.WithContext(ctx).Warn("putting message with id into retry")

		if err := p.retryHandler.Put(ctx, retryMsg); err != nil {
			return fmt.Errorf("can not write msg to output: %w", err)
		}
		p.writeMetricRetryCount(metricNameProducerRetryPutCount)
	}
...
func (p *producerRetryDaemon) ingestData(ctx context.Context) error {
	defer p.logger.Debug("ingestData is ending")
...
func (p *producerRetryDaemon) ingestDataFromSource(input Input, src string) func(ctx context.Context) error {
	...
	return func(ctx context.Context) error {
		defer p.logger.Debug("ingestDataFromSource %s is ending", src)
		defer p.stopIncomingData()
		...
		for {
			...
			case msg, ok := <-input.Data():
				...
				if retryId, ok := msg.Attributes[AttributeRetryId]; ok {
					...
					p.logger.WithContext(ctx).WithFields(log.Fields{
						"retry_id": fmt.Sprintf("%s", retryId),
					}).Warn("retrying message with id")
					p.writeMetricRetryCount(metricNameProducerRetryGetCount)
				}
...
func (p *producerRetryDaemon) stopIncomingData() {
	p.stopped.Do(func() {
		defer p.logger.Debug("stopIncomingData is ending")

		p.retryInput.Stop()
	})
}

func (p *producerRetryDaemon) writeMetricRetryCount(metricName string) {
	p.metricWriter.Write(metric.Data{
		&metric.Datum{
...
func (p *producerRetryDaemon) stopConsuming() error {
	p.stopIncomingData()
	return nil
}
